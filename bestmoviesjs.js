// JS CODE FOR MAIN (BESTMOVIES)
// Last update : 24/10/2021

window.onscroll = function() {funcStick()};

var header = document.getElementById("stick-header");
...
function JSONToHTMLTable(jsonData, elementToBind) {
  ...
  let displayColumns = ['poster', 'title'];
  ...
  for (var i = 0; i < jsonData.length; i++) {
    ...
    for (var j = 0; j < col.length; j++) {
      ...
      if(j == 0)
        ...
        if(col[j] == "poster") {
          var sourceImg = '<img src=' + '\"./data/movie-json-data-master/img/' + jsonData[i][col[j]] + '\" width=\"100em\" height=\"auto\">';
          jsonData[i][col[j]] = sourceImg;
        }
        if(col[j] == "title") {
          let str = jsonData[i][col[j]];
          str = str.replaceAll(' ', '_');
          var link = '<a href=\"./details.html?title=' + str + '\" style=\"text-decoration: none; color: darkred;\" title=\"./details.html?title=' + str + '\">' + jsonData[i][col[j]] + '</a>';
          jsonData[i][col[j]] = link;
        }
        tabCell.innerHTML = jsonData[i][col[j]];
      }

      if(j == 0) {
        tabCell.style.width = '1%';

      }
      else if(j == 1)
        tabCell.style.width = '10%';
      else
        tabCell.style.width = '79%';
    }
  }
